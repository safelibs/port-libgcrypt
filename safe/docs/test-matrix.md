...
## Phase 11 Regression Manifest

Phase 11 adds `safe/tests/regressions/manifest.json` as the catch-all regression index and `safe/scripts/run-regression-tests.sh --all` as the only phase 11 regression entrypoint. The `dependent-image-current-phase-tag` regression locks the dependent-image package/image rebuild path to the current local phase tag so phase 11 `.deb` manifests and image metadata are not compared against the phase 10 tag. The `dependent-image-apt-snapshot-retry` regression covers Docker apt setup against the pinned Noble snapshot: the rendered image build must use bounded snapshot retries, treat partial apt index updates as errors, prove the pinned `ca-certificates=20240203` and requested dependent package versions are visible instead of accepting partial indexes after transient 503s, and install the committed locked package closure from the pinned snapshot before the installed-package validator runs. The regression also rejects live-mirror, Launchpad, local `.deb`, and generated-package fallbacks, including fabricated `linux-libc-dev` payloads, because those paths would mask the phase 10 package closure instead of consuming the committed snapshot metadata exactly.
