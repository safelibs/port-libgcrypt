...
fn ghash_with_lengths(h: u128, aad: &[u8], ciphertext_bits: u64) -> u128 {
...
fn ghash_blocks(h: u128, mut y: u128, data: &[u8]) -> u128 {
...
fn ghash_block(h: u128, y: u128, block: &[u8]) -> u128 {
    let mut padded = [0u8; 16];
    padded.copy_from_slice(block);
    gf128_mul(y ^ u128::from_be_bytes(padded), h)
}

fn gf128_mul(x: u128, mut y: u128) -> u128 {
...
fn gost28147_sbox_from_oid(oid: &[u8]) -> Option<&'static [[u8; 16]; 8]> {
    match oid {
        b"1.2.643.2.2.30.0" => Some(&GOST28147_TEST_3411_SBOX),
        b"1.2.643.2.2.30.1" => Some(&GOST28147_CRYPTOPRO_3411_SBOX),
        b"1.2.643.2.2.31.0" => Some(&GOST28147_TEST_89_SBOX),
        b"1.2.643.2.2.31.1" => Some(&GOST28147_CRYPTOPRO_A_SBOX),
        b"1.2.643.2.2.31.2" => Some(&GOST28147_CRYPTOPRO_B_SBOX),
        b"1.2.643.2.2.31.3" => Some(&GOST28147_CRYPTOPRO_C_SBOX),
        b"1.2.643.2.2.31.4" => Some(&GOST28147_CRYPTOPRO_D_SBOX),
        b"1.2.643.7.1.2.5.1.1" => Some(&GOST28147_TC26_Z_SBOX),
        _ => None,
    }
}
...
fn aes_encrypt_block(key: &[u8], input: &[u8; 16]) -> Option<[u8; 16]> {
    ...
    let nr = nk + 6;
    let words = aes_expand_key(key, nk, nr);
    let mut state = *input;
    aes_add_round_key(&mut state, &words, 0);
    for round in 1..nr {
        aes_sub_bytes(&mut state);
        aes_shift_rows(&mut state);
        aes_mix_columns(&mut state);
        aes_add_round_key(&mut state, &words, round);
    }
    aes_sub_bytes(&mut state);
    aes_shift_rows(&mut state);
    aes_add_round_key(&mut state, &words, nr);
    Some(state)
}

fn aes_expand_key(key: &[u8], nk: usize, nr: usize) -> Vec<u32> {
