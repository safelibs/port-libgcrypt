...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let generated_export_source = generate_export_source(&symbols)?;
...
fn generate_export_source(symbols: &[String]) -> io::Result<String> {
    ...
    let missing: Vec<&str> = symbols
        .iter()
        .map(String::as_str)
        .filter(|symbol| !handwritten.contains(symbol))
        .collect();
    if !missing.is_empty() {
        return Err(io::Error::other(format!(
            "missing handwritten public exports: {}",
            missing.join(", ")
        )));
    }

    Ok("/* @generated by build.rs: public exports are provided by Rust and safe/cabi/exports.c. */\n"
        .to_string())
}
