...
| `-lgpg-error` in `safe/debian/rules:44` and `safe/scripts/build-release-lib.sh:38` | libgpg-error shared library | The final shared object keeps libgcrypt-compatible error-code conversion and message lookup linked against the system libgpg-error ABI. | Replace with a generated Rust error table only if packaging no longer links libgpg-error and compatibility with system error sources is preserved. |
| `-lgmp` in `safe/debian/rules:44` and `safe/scripts/build-release-lib.sh:38` | GMP shared library | The final shared object resolves GMP-backed MPI arithmetic, import/export, prime, and public-key helper calls declared in `safe/src/mpi/mod.rs`. | Replace with a reviewed Rust big-integer backend that preserves MPI ABI behavior and constant-time secret operations. |
| `-lpthread` in `safe/debian/rules:44` and `safe/scripts/build-release-lib.sh:38` | POSIX threads runtime library | The Debian and local link steps keep the shared-library system link closure compatible with threaded C callers and linked runtime support. | Remove only after a symbol-level link audit proves no pthread runtime support is required by the produced shared object or its C ABI contract. |
| `-lm` in `safe/debian/rules:44` and `safe/scripts/build-release-lib.sh:38` | libc math library | The final shared-library link includes the traditional libgcrypt system math dependency in the package and local ABI-test artifact. | Remove only after a linker audit proves no math-library symbols are needed on supported targets. |
| `-lc` in `safe/debian/rules:44` and `safe/scripts/build-release-lib.sh:38` | libc runtime library | The final shared object depends on libc for allocation, errno, stdio, OS, and C shim surfaces that remain outside the safe Rust boundary. | Replace individual libc calls with safe Rust wrappers where possible; `-lc` can only disappear if every remaining C/libc surface is removed. |
| `-lgcc_s` in `safe/debian/rules:44` and `safe/scripts/build-release-lib.sh:38` | GCC support runtime library | The explicit shared-library link keeps compiler/runtime support available for the mixed Rust/C static archive link. | Remove only after supported-toolchain link testing proves the produced shared object has no GCC support-runtime dependency. |
| Helper-binary `unsafe extern "C"` declarations in `safe/src/bin/dumpsexp.rs:7`, `safe/src/bin/hmac256.rs:7`, and the libgcrypt functions in `safe/src/bin/mpicalc.rs:9` through `safe/src/bin/mpicalc.rs:24` | The installed libgcrypt ABI produced by this port | The helper tools intentionally exercise the installed ABI rather than reaching into Rust internals. | Use safe Rust crate calls only if the tools stop being ABI compatibility tools; otherwise keep the self-call classification. |
| `static mut stdout` in `safe/src/bin/mpicalc.rs:7` | libc stdio ABI | `mpicalc --print-config` passes the process `stdout` `FILE *` to `gcry_control(GCRYCTL_PRINT_CONFIG, stdout)` at `safe/src/bin/mpicalc.rs:223`. This is libc stdio, not a libgcrypt ABI self-call. | Avoid the C `FILE *` path by printing the string returned from `gcry_get_config` if the helper no longer needs to mirror the upstream `gcry_control` config-print path. |
